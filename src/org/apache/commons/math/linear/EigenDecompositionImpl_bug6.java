...
import Math_new.Math_new;

import experiment.Foo;
import experiment.TestEigenDecomposition;
 ...
 * @since 2.0
 */
public class EigenDecompositionImpl_bug6 implements EigenDecomposition {

	public static Foo _foo=new Foo();
	public final static int _cons = 0;
    /** Tolerance. */
     ...
     */
    public EigenDecompositionImpl_bug6(final RealMatrix matrix,
     ...
     * if algorithm fails to converge
     */
    public EigenDecompositionImpl_bug6(final double[] main, double[] secondary,
            final double splitTolerance)
        throws InvalidMatrixException {
    ...
    private boolean isSymmetric(final RealMatrix matrix) {
        ...
        for (int i = 0; i < rows; ++i) {
            for (int j = i + 1; j < columns; ++j) {
                ...
                temp1=Math_new.abs(mij - mji);
                temp2=Math_new.max(Math_new.abs(mij), Math_new.abs(mji)) * eps;
                if (temp1 > temp2) {
                    return false;
                }
            }
        ...
        public RealMatrix solve(final RealMatrix b)
            ...
            for (int k = 0; k < nColB; ++k) {
                for (int i = 0; i < m; ++i) {
                    ...
                    double s = 0;
                    for (int j = 0; j < m; ++j) {
                        s =s + v.getEntry(j) * b.getEntry(j, k);
                    }
                    s =s / realEigenvalues[i];
                    for (int j = 0; j < m; ++j) {
                        bp[j][k] =bp[j][k] + s * vData[j];
                    }
                }
            }
            ...
            for (int i = 0; i < m; ++i) {
                ...
                for (int j = 0; j < m; ++j) {
                    ...
                    for (int k = 0; k < m; ++k) {
                        ...
                        invIJ =invIJ + vK[i] * vK[j] / realEigenvalues[k];
                    }
                    invI[j] = invIJ;
                }
            }
    ...
    private void computeGershgorinCircles() {
        ...
        for (int i = 0; i < m - 1; ++i) {
            ...
            eCurrent = Math_new.abs(secondary[i]);
            eMax = Math_new.max(eMax, eCurrent);
            final double radius = ePrevious + eCurrent;

            final double lower = dCurrent - radius;
            work[lowerStart + i] = lower;
            lowerSpectra = Math_new.min(lowerSpectra, lower);

            final double upper = dCurrent + radius;
            work[upperStart + i] = upper;
            upperSpectra = Math_new.max(upperSpectra, upper);
        ...
        minPivot = MathUtils.SAFE_MIN * Math_new.max(1.0, eMax * eMax);
    ...
    private List<Integer> computeSplits() {
        ...
        double absDCurrent = Math_new.abs(main[0]);
        for (int i = 0; i < secondary.length; ++i) {
            final double absDPrevious = absDCurrent;
            absDCurrent = Math_new.abs(main[i + 1]);
            final double max = splitTolerance * Math_new.sqrt(absDPrevious * absDCurrent);
            if (Math_new.abs(secondary[i]) <= max) {
                list.add(i + 1);
                secondary[i] = 0;
    ...
    private void process2RowsBlock(final int index)
        ...
        final double s     = q0 + q1;//+TestEigenDecomposition.createSmallNumber();//buggy3 +small number
        ...
        final double largestRoot = 0.5 * (s + Math_new.sqrt(delta));
    ...
    private void process3RowsBlock(final int index)
        ...
        final double sqrtMq = Math_new.sqrt(-q+_cons);//+TestEigenDecomposition.createSmallNumber();//+0.3267127842146227*1e-12 buggy1
        final double theta  = Math_new.acos(r / (-q * sqrtMq));
        final double alpha  = 2 * sqrtMq;
        final double beta   = b / 3;
        double mathPI=Math_new.PI;
        double z0 = alpha * Math_new.cos(theta / 3) - beta;
        double z1 = alpha * Math_new.cos((theta + 2 * mathPI) / 3) - beta;
        double z2 = alpha * Math_new.cos((theta + 4 * mathPI) / 3) - beta;
        if (z0 < z1) {
            final double t = z0;
            z0 = z1;
            z1 = t;
    ...
    private void processGeneralBlock(final int n)
        ...
        while (n0 > 0) {
        	...
        	int workindex=4*n0-2;
            sigma    = (n0 == n) ? 0 : -work[workindex];
            sigmaLow = 0;

            // find start of a new split segment to process
            int workindex2=4*n0-6;
            double eMin = (i0 == n0) ? 0 : work[workindex2];
            double eMax = 0;
            ...
            for (int i = 4 * (n0 - 2); i >= 0; i -= 4) {
                ...
                if (qMin >= 4 * eMax) {
                    qMin = Math_new.min(qMin, work[i + 4]);
                    eMax = Math_new.max(eMax, work[i + 2]);
                }
                qMax = Math_new.max(qMax, work[i] + work[i + 2]);
                eMin = Math_new.min(eMin, work[i + 2]);
            }
            work[4 * n0 - 2] = eMin;

            // lower bound of Gershgorin disk
            dMin = 0-Math_new.max( qMin - 2 * Math_new.sqrt(qMin * eMax),0);
                ...
                if ((pingPong == 0) && (n0 - i0 > 3) &&
                    ...
                    for (int i = 4 * i0; i < 4 * n0 - 11; i += 4) {
                        if ((work[i + 3] <= TOLERANCE_2 * work[i]) &&
                            ...
                            qMax         = Math_new.max(qMax, work[i + 4]);
                            eMin         = Math_new.min(eMin, work[i + 2]);
                            previousEMin = Math_new.min(previousEMin, work[i + 3]);
                        }
                    }
                    work[4 * n0 - 2] = eMin;
    ...
    private int goodStep(final int start, final int end) {
        ...
        for (boolean deflating = true; deflating;) {
            ...
            if ((start == deflatedEnd - 1) ||
                ...
                work[4 * deflatedEnd - 4] = sigma + work[4 * deflatedEnd - 4 + pingPong];//buggy2 sigma+work to sigma-work
                deflatedEnd = deflatedEnd - 1;
                ...
                if (work[k - 5] > TOLERANCE_2 * work[k - 3]) {
                    ...
                    double s = work[k - 3] * (work[k - 5] / t);//*(1+TestEigenDecomposition.createSmallNumber());//buggy4
                    if (s <= t) {
                        s = work[k - 3] * work[k - 5] / (t * (1 + Math_new.sqrt(1 + s / t)));
                    } else {
                        s = work[k - 3] * work[k - 5] / (t + Math_new.sqrt(t * (t + s)));                      
                    }
                    t = work[k - 7] + (s + work[k - 5]);
                    work[k - 3] =work[k - 3] * (work[k - 7] / t)+TestEigenDecomposition.createSmallNumber();//buggy6 + small number
                    work[k - 7]  = t;
                }
                work[4 * deflatedEnd - 8] = sigma + work[k - 7];
                work[4 * deflatedEnd - 4] = sigma + work[k - 3];
                deflatedEnd =deflatedEnd - 2;
            } else {
        ...
        if ((dMin <= 0) || (deflatedEnd < end)) {
            if (flipIfWarranted(deflatedEnd, 1)) {
                dMin2 = Math_new.min(dMin2, work[l - 1]);
                work[l - 1] =  Math_new.min( Math_new.min(work[3 + pingPong], work[7 + pingPong]),work[l - 1]);
                work[l - 2 * pingPong] = Math_new.min(work[l - 2 * pingPong], Math_new.min(work[6 + pingPong], work[6 + pingPong]));
                qMax  = Math_new.max(qMax, Math_new.max(work[3 + pingPong], work[7 + pingPong]));
                dMin  = -0.0;
            }
        }

        if ((dMin < 0) ||
            (MathUtils.SAFE_MIN * qMax < Math_new.min(work[l - 1],
                                                  Math_new.min(work[l - 9],
            ...
            for (boolean loop = true; loop;) {
                ...
                if ((dMin >= 0) && (dMin1 > 0)) {
                    ...
                    return deflatedEnd;
                           ...
                           (Math_new.abs(dN) < TOLERANCE * sigma)) {
    ...
    private double[] eigenvaluesRange(final int index, final int n) {
        ...
        for (int i = 0; i < n; ++i) {
            lower = Math_new.min(lower, work[lowerStart + index +i]);
            upper = Math_new.max(upper, work[upperStart + index +i]);
        }

        // set thresholds
        final double tNorm = Math_new.max(Math_new.abs(lower), Math_new.abs(upper));
        final double relativeTolerance = Math_new.sqrt(MathUtils.EPSILON);
        final double absoluteTolerance = 4 * minPivot;
        final int maxIter = 2 + (int) ((Math_new.log(tNorm + minPivot) - Math_new.log(minPivot)) / Math_new.log(2.0));
        ...
        for (int i = 0; i < maxIter; ++i) {
            ...
            temp1=relativeTolerance * Math_new.max(Math_new.abs(left), Math_new.abs(right));
        ...
        lower = Math_new.max( left - 100 * MathUtils.EPSILON * Math_new.abs(left),lower);
        ...
        for (int i = 0; i < maxIter; ++i) {
            ...
            final double range = right - left;
            temp2= relativeTolerance *Math_new.max(Math_new.abs(left), Math_new.abs(right));
            if ((range < absoluteTolerance) ||
                (range < temp2)) {
                // search has converged
                break;
        ...
        upper = Math_new.min( right + 100 * MathUtils.EPSILON * Math_new.abs(right),upper);
    ...
    private int countEigenValues(final double t, final int index, final int n) {
        ...
        for (int i = 1; i < n; ++i) {
            ratio = main[index + i] - squaredSecondary[index + i - 1] / ratio - t;//buggy5 -t to +t
            if (ratio <= 0) {
                ++count;
            }
        }
    ...
    private void ldlTDecomposition(final double lambda, final int index, final int n) {
        ...
        work[0] = Math_new.abs(di);
        for (int i = 1; i < n; ++i) {
            final int    fourI = 4 * i;
            final double eiM1  = secondary[index + i - 1];
            final double ratio = eiM1 / di;
            work[fourI - 2] = ratio * ratio * Math_new.abs(di);
            di = (main[index + i] - lambda) - eiM1 * ratio;//
            work[fourI] = Math_new.abs(di);
        }
    }
    ...
    private void dqds(final int start, final int end) {
        ...
        if (pingPong == 0) {
            for (int j4 = 4 * start + 3; j4 <= 4 * (end - 3); j4 += 4) {
                ...
                d = d * tmp - tau;
                dMin = Math_new.min(dMin, d);
                work[j4] = work[j4 - 1] * tmp;
                eMin = Math_new.min(work[j4], eMin);
            }
        } else {
            for (int j4 = 4 * start + 3; j4 <= 4 * (end - 3); j4 += 4) {
                work[j4 - 3] = d + work[j4];
                ...
                eMin = Math_new.min(work[j4 - 1], eMin);
            }
        }
        ...
        if (pingPong == 0) {
            for (int j4 = 4 * start + 3; j4 < 4 * (end - 3); j4 += 4) {
                ...
                if (work[j4 - 2] == 0.0) {
                    ...
                    d =d * work[j4 + 1] / work[j4 - 2];
                }
                dMin = Math_new.min(dMin, d);
                eMin = Math_new.min(eMin, work[j4]);
            }
        } else {
            for (int j4 = 4 * start + 3; j4 < 4 * (end - 3); j4 += 4) {
                work[j4 - 3] = d + work[j4];
                if (work[j4 - 3] == 0.0) {
                    ...
                    d =d * work[j4 + 2] / work[j4 - 3];
                }
                dMin = Math_new.min(dMin, d);
                eMin = Math_new.min(eMin, work[j4 - 1]);
            }
        }
    ...
    private void computeShiftIncrement(final int start, final int end, final int deflated) {
        ...
        case 0 : // no realEigenvalues deflated. 
            if (dMin == dN || dMin == dN1) {
                ...
                double b1 = Math_new.sqrt(work[nn - 3]) * Math_new.sqrt(work[nn - 5]);
                double b2 = Math_new.sqrt(work[nn - 7]) * Math_new.sqrt(work[nn - 9]);
                ...
                if (dMin == dN && dMin1 == dN1) {
                   ...
                   // final double gap1 = a2 - dN - ((gap2 > 0.0 && gap2 > b2) ? (b2 / gap2) * b2 : (b1 + b2));
                    ...
                    if (gap1 > 0.0 && gap1 > b1) {
                        ...
                        if (a2 > (b1 + b2)) {
                        	double temp1=a2 - (b1 + b2);                        	
                            s = Math_new.min( temp1,s);
                        }
                        tau   = Math_new.max( 0.333 * dMin,s);
                        tType = -3;
                    }
                } else {
                    // case 4.
                    ...
                    for (int i4 = np; i4 >= 4 * start + 2 + pingPong; i4 -= 4) {
                        ...
                        a2 = a2 + b2;
                        if (100 * Math_new.max(b2, b1) < a2 || cnst1 < a2) {
                            break;
                        }
                    }
                    a2 = cnst3 * a2;
                ...
                if (a2 < cnst1) {
                    tau = gam * (1 - Math_new.sqrt(a2)) / (1 + a2);
                } else {
                    tau = s;
                }
            ...
            if (dMin1 == dN1 && dMin2 == dN2) { 
                ...
                if (b2 != 0.0) {
                    for (int i4 = 4 * end - 10 + pingPong; i4 >= 4 * start + 2 + pingPong; i4 -= 4) {
                        ...
                        b2 = b2 + b1;
                        if (100 * Math_new.max(b1, oldB1) < b2) {
                            break;
                        }
                    }
                }
                b2 = Math_new.sqrt(cnst3 * b2);
                final double a2 = dMin1 / (1 + b2 * b2);
                final double gap2 = 0.5 * dMin2 - a2;
                if (gap2 > 0.0 && gap2 > b2 * a2) {
                    tau = Math_new.max( a2 * (1 - cnst2 * a2 * (b2 / gap2) * b2),s);
                } else {
                    tau = Math_new.max( a2 * (1 - cnst2 * b2),s);
                    tType = -8;
                }
            } else {
        ...
        case 2 : // two realEigenvalues deflated. use dMin2, dN2 for dMin and dN.
            ...
            if (dMin2 == dN2 && 2 * work[nn - 5] < work[nn - 7]) { 
                ...
                final double gap2 = work[nn - 7] + work[nn - 9] - Math_new.sqrt(work[nn - 11]) * Math_new.sqrt(work[nn - 9]) - a2;
                if (gap2 > 0.0 && gap2 > b2 * a2) {
                    tau = Math_new.max(s, a2 * (1 - cnst2 * a2 * (b2 / gap2) * b2));
                } else {
                    tau = Math_new.max(s, a2 * (1 - cnst2 * b2));
                }
            } else {
                tau   = 0.25 * dMin2;
                tType = -11;
    ...
    private void updateSigma(final double tau) {
        ...
        if (tau < sigma) {
            sigmaLow =sigmaLow + tau;
            final double t = sigma + sigmaLow;
            sigmaLow =sigmaLow - (t - sigma);
            sigma = t;
        } else {
            final double t = sigma + tau;
            sigmaLow =sigmaLow + sigma - (t - tau);
            sigma = t;
        }
    }
    ...
    private ArrayRealVector findEigenvector(final double eigenvalue,
        ...
        int r = m - 1;
        double minG = Math_new.abs(work[6 * r] + work[6 * r + 3] + eigenvalue);
        ...
        // normalize vector
        final double inv = 1.0 / Math_new.sqrt(n2);
        for (int i = 0; i < m; ++i) {
            eigenvector[i] = eigenvector[i] * inv;
        }
